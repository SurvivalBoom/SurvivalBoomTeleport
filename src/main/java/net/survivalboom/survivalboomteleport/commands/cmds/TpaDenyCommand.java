...
public class TpaDenyCommand {
    ...
    public static void command(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (!sender.hasPermission("sbteleport.tpdeny")) {
            PluginMessages.sendMessage(sender, PluginMessages.getMessage("no-permission").replace("{PERMISSION}", "sbteleport.tpdeny"));
            return;
        }
